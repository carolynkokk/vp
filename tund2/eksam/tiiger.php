...
if(isset($_POST["commentsubmit"]) and !empty($_POST["kommentaar"])){
    ...
    $stmt = $conn->prepare("INSERT INTO tigercomment (kommentaar, autor) VALUES (?, ?)");
    //echo $conn->error;
...
$conn = new mysqli($serverhost, $serverusername, $serverpassword, $database);
$stmt = $conn->prepare("SELECT autor, kommentaar FROM tigercomment");
//echo $conn->error;
