<?php
  ...
  if(isset($_POST["studiosubmit"])) {
	...
	if(empty($studioerror)) {
		$notice = savestudio($_POST["studionameinput"], $_POST["studioaddressinput"]);
	}
  }
  
  if(isset($_POST["filmsubmit"])) {
	  if(empty($_POST["filmtitleinput"])) {
		  $filmtitleerror = "Filmi nimi on sisestamata!";
		 }
		 else {
			 $title = test_input($_POST["filmtitleinput"]);
			 }
			 
		 if(empty($_POST["durationinput"])) {
		  $filmdurationerror = "Filmi kestus on sisestamata!";
		 }
		 else {
			 $duration = intval($_POST["durationinput"]);
			 }
		if(empty($_POST["productionyearinput"])) {
		  $productionyearerror = "Filmi valmimisaasta on sisestamata!";
		 }
		 else {
			 $productionyear = intval($_POST["productionyearinput"]);
			 }
		if(empty($filmtitleerror) and empty($filmdurationerror) and empty($productionyearerror)) {
			$description = test_input($_POST["descriptioninput"]);
      $result = savefilm($title, $productionyear, $duration, $description);
      if($result == "ok") {
        $filmnotice = "Film salvestatud!";
        $title = "";
        $duration = null;
  ...
  <h2>Lisa film</h2>
  <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">
	<label for="filmtitleinput">Filmi nimi</label>
	<br />
	<input type="text" name="filmtitleinput" id="filmtitleinput" placeholder="Filmi nimi" value="<?php echo $title; ?>">
	<span><?php echo $filmtitleerror; ?></span>
	<br />
	...
	<span><?php echo $filmdurationerror; ?></span>
	<br />
	<label for="productionyearinput">Filmi valmimisaasta</label>
	<br /> 
	...
	<br />
	<textarea rows="5" cols="20" name="descriptioninput" id="descriptioninput" placeholder="Lühikirjeldus: "><?php echo $description; ?></textarea>
	<br />
